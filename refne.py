...
def synthetic_data_generator_old(in_df, n_samples, discrete=None):
    """
    ...
    :param discrete: lost of the columns of the input dataframe containing categorical variables. Synthetic data
    generated from discrete variables consists of a random selection of the recorded values. Synthetic data from
    continuous variables are instead generated by randomly selecting a value from a uniform distribution having as min
    and max parameters the extreme values of these variables.
    :return: out_df: Pandas dataframe of synthetic data
    """
    if discrete is None:
        discrete = []
    out_df = pd.DataFrame()
    for column in in_df.columns.tolist():
        if column in discrete:
            out_df[column] = np.random.choice(np.unique(in_df[column]).tolist(), n_samples)
        else:
            minvalue = in_df[column].min()
            maxvalue = in_df[column].max()
            out_df[column] = np.round(np.random.uniform(minvalue, maxvalue, n_samples), 4)
    return out_df
